				...
				<div class="up">
					<div class="smallbox" style="width: 500px; height:700px;">
						<form action="./editboard" method="post" enctype="multipart/form-data">
							<div id="preview">
								<img id="previewImg" src="/FinalProject0430/${b.filename}"
									width="500px" height="300px">
							</div>
							<input type="file" name="uploadFile" id="uploadFile"accept="image/*" /> 
							<input type="hidden" value="0" name="uploadEx" id="uploadEx"/> 
							<br> 
							<br> 
							<select name="food">
								<option value="" ${b.food eq "" ? "selected":"" }>음식 선택</option>
								<option value="achicken"
									${b.food eq "achicken" ? "selected":"" }>치킨</option>
								<option value="bpizza" ${b.food eq "bpizza" ? "selected":"" }>피자</option>
								<option value="cgogi" ${b.food eq "cgogi" ? "selected":"" }>고기</option>
								<option value="dzzim" ${b.food eq "dzzim" ? "selected":"" }>찜·탕·찌개</option>
								<option value="ejok" ${b.food eq "ejok" ? "selected":"" }>족발·보쌈</option>
								<option value="fkaz" ${b.food eq "fkaz" ? "selected":"" }>돈까스</option>
								<option value="gjapanesef"
									${b.food eq "gjapanesef" ? "selected":"" }>회·일식</option>
								<option value="hkoreanf"
									${b.food eq "hkoreanf" ? "selected":"" }>한식</option>
								<option value="ichinesef"
									${b.food eq "ichinesef" ? "selected":"" }>중식</option>
								<option value="jyangf" ${b.food eq "jyangf" ? "selected":"" }>양식</option>
								<option value="kfastf" ${b.food eq "kfastf" ? "selected":"" }>패스트푸드</option>
								<option value="lcafe" ${b.food eq "lcafe" ? "selected":"" }>카페</option>
							</select> <br> <br>
							<div style="display: flex; align-items: center;">
								...
								<div style="flex-grow: 1;">
									<input class="input" type="text" name="title"
										value="${b.title }"
										style="width: 90%; height: 20px; padding: 3px;">
								</div>
							</div>
							<br> <br>
							<div style="display: flex; align-items: center;">
								...
								<div style="flex-grow: 1;">
									<input class="input" name="addr" value="${b.addr }"
										style="width: 90%; height: 20px; padding: 3px;">
								</div>
							</div>
							<br> <br>
							<div style="display: flex; align-items: center;">
								...
								<div style="flex-grow: 1;">
									<input class="input" name="phone" value="${b.phone}"
										style="width: 90%; height: 20px; padding: 3px;"> <input
										class="input" name="num" type="hidden" value="${b.num}">
								</div>
							</div>
							<br> <br>
							<div style="display: flex; align-items: center;">
								<label for="hm" style="margin-left: 10px; margin-right: 10px;">영업시간:</label>
								<select id="HH" name="HH" required></select> <label for="HH">:</label>
								<select id="mm" name="mm" required></select> <label for="mm"></label>
								<p>~</p>
								<select id="HH2" name="HH2" required></select> <label for="HH2">:</label>
								<select id="mm2" name="mm2" required></select> <label for="mm2"></label>
							</div>
							<br>
							<div style="text-align: center; margin-bottom: 30px;">
								<button class="log" type="submit">수정</button>
...
<script src="http://code.jquery.com/jquery-latest.js"></script>
	...
	for (var HH = 0; HH <= 24; HH++) {
	    ...
	    option.value = HH < 10 ? "0" + HH : HH; // 시간이 한 자리수일 때는 앞에 0을 붙입니다.
	...
	for (var mm = 0; mm <= 59; mm++) {
	    var option = document.createElement("option");
	    option.value = mm < 10 ? "0" + mm : mm; // 분이 한 자리수일 때는 앞에 0을 붙입니다.
	...
	for (var HH2 = 0; HH2 <= 24; HH2++) {
	    var option = document.createElement("option");
	    option.value = HH2 < 10 ? "0" + HH2 : HH2; // 시간이 한 자리수일 때는 앞에 0을 붙입니다.
	    option.text = option.value + "시";
	...
	for (var mm2 = 0; mm2 <= 59; mm2++) {
	    var option = document.createElement("option");
	    option.value = mm2 < 10 ? "0" + mm2 : mm2; // 분이 한 자리수일 때는 앞에 0을 붙입니다.
	    option.text = option.value + "분";
	...
	var start2 = "${start2}";
	var start3 = "${start3}";
	document.getElementById("HH").value = start0;
	document.getElementById("HH2").value = start2;
	document.getElementById("mm").value = start1;
	document.getElementById("mm2").value = start3;
    ...
    function loginPage() {
		...
		// 수정 ( 기존의 파일을 그대로 쓴다(0) -> 업로드 / 파일이름만 기존(ㅇ옛날 저장) / 1새로 업로드 -> 새롭게 다시 업로드 필요)
